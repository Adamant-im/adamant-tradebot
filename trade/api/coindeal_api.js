...
module.exports = function() {
  ...
  const EXCHANGE_API = {
    ...
    /**
     * Get trades history
     * @param pair Trading pair, like BTCUSDT
     * @param {Number} limit Default: 100. — It is wrong, tested: max limit = 20
     * @param {String} sort Enum: "DESC", "ASC". Default: "DESC"
     * @param {String} by Enum: "timestamp", "id". Default: "timestamp". Filter field: timestamp in milliseconds or id
     * @param {String} from If filtered by timestamp, then timestamp in millisecond otherwise trade id. Example: from=1572356518965
     * @param {String} till If filtered by timestamp, then timestamp in millisecond otherwise trade id. Example: from=1572356518965
     * @param {Number} offset Example: offset=10
     * @return {Array of Object} Last trades
     */
    getTradesHistory: function(symbol, limit, sort, by, from, till, offset) {
      const data = {};
      data.symbol = symbol;
      ...
      if (till) data.till = till;
      if (offset) data.offset = offset;
      return protectedRequest(`/api/v2/history/trades`, data, 'get');
    },
